#include "../include/sm4.h"
#include <string.h>

// SM4 S盒
static const uint8_t sm4_sbox_table[256] = {
    
};

// FK参数
static const uint32_t sm4_fk[4] = {
    0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc
};

// CK参数
static const uint32_t sm4_ck[32] = {
    0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269,
    0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
    0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249,
    0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
    0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229,
    0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
    0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209,
    0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279
};

// S盒替换
uint8_t sm4_sbox(uint8_t a) {
    return sm4_sbox_table[a];
}

// 非线性变换τ
uint32_t sm4_tau(uint32_t a) {
    // 正确的字节提取方式，不依赖字节序
    uint8_t b0 = (uint8_t)(a >> 24);
    uint8_t b1 = (uint8_t)(a >> 16);
    uint8_t b2 = (uint8_t)(a >> 8);
    uint8_t b3 = (uint8_t)a;
    
    return ((uint32_t)sm4_sbox(b0) << 24) |
           ((uint32_t)sm4_sbox(b1) << 16) |
           ((uint32_t)sm4_sbox(b2) << 8) |
           (uint32_t)sm4_sbox(b3);
}

// 线性变换L
uint32_t sm4_linear_transform(uint32_t b) {
    return b ^ ROTL(b, 2) ^ ROTL(b, 10) ^ ROTL(b, 18) ^ ROTL(b, 24);
}

// 密钥扩展线性变换L'
uint32_t sm4_linear_transform_key(uint32_t b) {
    return b ^ ROTL(b, 13) ^ ROTL(b, 23);
}

// 合成置换T
static uint32_t sm4_T(uint32_t x) {
    return sm4_linear_transform(sm4_tau(x));
}

// 密钥扩展合成置换T'
static uint32_t sm4_T_key(uint32_t x) {
    return sm4_linear_transform_key(sm4_tau(x));
}

// SM4轮函数
uint32_t sm4_round_function(uint32_t x0, uint32_t x1, uint32_t x2, uint32_t x3, uint32_t rk) {
    return x0 ^ sm4_T(x1 ^ x2 ^ x3 ^ rk);
}

// SM4密钥扩展
void sm4_setkey_enc(uint32_t rk[SM4_ROUNDS], const uint8_t key[SM4_KEY_SIZE]) {
    uint32_t k[4];
    uint32_t mk[4];
    int i;

    // 将密钥转换为4个32位字
    k[0] = get_uint32_be(key);
    k[1] = get_uint32_be(key + 4);
    k[2] = get_uint32_be(key + 8);
    k[3] = get_uint32_be(key + 12);

    // 与FK进行异或
    mk[0] = k[0] ^ sm4_fk[0];
    mk[1] = k[1] ^ sm4_fk[1];
    mk[2] = k[2] ^ sm4_fk[2];
    mk[3] = k[3] ^ sm4_fk[3];

    // 生成轮密钥
    for (i = 0; i < SM4_ROUNDS; i++) {
        rk[i] = mk[0] ^ sm4_T_key(mk[1] ^ mk[2] ^ mk[3] ^ sm4_ck[i]);
        mk[0] = mk[1];
        mk[1] = mk[2];
        mk[2] = mk[3];
        mk[3] = rk[i];
    }
}

// 生成解密轮密钥
void sm4_setkey_dec(uint32_t dk[SM4_ROUNDS], const uint32_t ek[SM4_ROUNDS]) {
    int i;
    for (i = 0; i < SM4_ROUNDS; i++) {
        dk[i] = ek[SM4_ROUNDS - 1 - i];
    }
}

// SM4分组加密/解密
void sm4_crypt_block(const sm4_context *ctx, const uint8_t input[SM4_BLOCK_SIZE], 
                     uint8_t output[SM4_BLOCK_SIZE]) {
    uint32_t x[4];
    uint32_t tmp;
    int i;

    // 输入转换
    x[0] = get_uint32_be(input);
    x[1] = get_uint32_be(input + 4);
    x[2] = get_uint32_be(input + 8);
    x[3] = get_uint32_be(input + 12);

    // 32轮迭代
    for (i = 0; i < SM4_ROUNDS; i++) {
        tmp = x[0] ^ sm4_T(x[1] ^ x[2] ^ x[3] ^ ctx->rk[i]);
        x[0] = x[1];
        x[1] = x[2];
        x[2] = x[3];
        x[3] = tmp;
    }

    // 反序输出
    put_uint32_be(output, x[3]);
    put_uint32_be(output + 4, x[2]);
    put_uint32_be(output + 8, x[1]);
    put_uint32_be(output + 12, x[0]);
}

// 初始化SM4上下文
int sm4_init(sm4_context *ctx, const uint8_t key[SM4_KEY_SIZE], int encrypt) {
    if (ctx == NULL || key == NULL) {
        return -1;
    }

    ctx->encrypt = encrypt;

    if (encrypt) {
        sm4_setkey_enc(ctx->rk, key);
    } else {
        uint32_t temp_rk[SM4_ROUNDS];
        sm4_setkey_enc(temp_rk, key);
        sm4_setkey_dec(ctx->rk, temp_rk);
    }

    return 0;
}

// ECB模式加密/解密
int sm4_crypt_ecb(const sm4_context *ctx, size_t length,
                  const uint8_t *input, uint8_t *output) {
    if (ctx == NULL || input == NULL || output == NULL) {
        return -1;
    }

    if (length % SM4_BLOCK_SIZE != 0) {
        return -1;  // 长度必须是16的倍数
    }

    while (length > 0) {
        sm4_crypt_block(ctx, input, output);
        input += SM4_BLOCK_SIZE;
        output += SM4_BLOCK_SIZE;
        length -= SM4_BLOCK_SIZE;
    }

    return 0;
}

// CBC模式加密/解密
int sm4_crypt_cbc(const sm4_context *ctx, int mode, size_t length,
                  uint8_t iv[SM4_BLOCK_SIZE], const uint8_t *input, uint8_t *output) {
    uint8_t temp[SM4_BLOCK_SIZE];
    size_t i;

    if (ctx == NULL || iv == NULL || input == NULL || output == NULL) {
        return -1;
    }

    if (length % SM4_BLOCK_SIZE != 0) {
        return -1;
    }

    if (mode == 1) { // 加密
        while (length > 0) {
            // 异或IV或前一个密文块
            for (i = 0; i < SM4_BLOCK_SIZE; i++) {
                temp[i] = input[i] ^ iv[i];
            }
            
            sm4_crypt_block(ctx, temp, output);
            
            // 更新IV
            memcpy(iv, output, SM4_BLOCK_SIZE);
            
            input += SM4_BLOCK_SIZE;
            output += SM4_BLOCK_SIZE;
            length -= SM4_BLOCK_SIZE;
        }
    } else { // 解密
        while (length > 0) {
            // 保存当前密文块作为下一轮的IV
            memcpy(temp, input, SM4_BLOCK_SIZE);
            
            sm4_crypt_block(ctx, input, output);
            
            // 异或IV
            for (i = 0; i < SM4_BLOCK_SIZE; i++) {
                output[i] ^= iv[i];
            }
            
            // 更新IV
            memcpy(iv, temp, SM4_BLOCK_SIZE);
            
            input += SM4_BLOCK_SIZE;
            output += SM4_BLOCK_SIZE;
            length -= SM4_BLOCK_SIZE;
        }
    }

    return 0;
}

// CTR模式加密/解密
int sm4_crypt_ctr(const sm4_context *ctx, size_t length, size_t *nc_off,
                  uint8_t nonce_counter[SM4_BLOCK_SIZE], uint8_t stream_block[SM4_BLOCK_SIZE],
                  const uint8_t *input, uint8_t *output) {
    uint8_t c;
    size_t n;

    if (ctx == NULL || nc_off == NULL || nonce_counter == NULL || 
        stream_block == NULL || input == NULL || output == NULL) {
        return -1;
    }

    n = *nc_off;

    if (n >= SM4_BLOCK_SIZE) {
        return -1;
    }

    while (length--) {
        if (n == 0) {
            sm4_crypt_block(ctx, nonce_counter, stream_block);
            
            // 递增计数器
            for (int i = SM4_BLOCK_SIZE - 1; i >= 0; i--) {
                if (++nonce_counter[i] != 0) {
                    break;
                }
            }
        }
        
        c = *input++;
        *output++ = c ^ stream_block[n];
        n = (n + 1) % SM4_BLOCK_SIZE;
    }

    *nc_off = n;
    return 0;
}
