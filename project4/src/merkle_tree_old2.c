...
int merkle_tree_build(merkle_tree_t *tree, const uint8_t **leaves, 
    ...
    // 计算根哈希
    compute_tree_hash(leaf_hashes, leaf_count, tree->root_hash);
    
    // 保存叶子哈希（用于生成证明）
    tree->nodes = (merkle_node_t*)malloc(leaf_count * sizeof(merkle_node_t));
    if (!tree->nodes) {
        for (size_t i = 0; i < leaf_count; i++) {
            free(leaf_hashes[i]);
        }
    ...
    for (size_t i = 0; i < leaf_count; i++) {
        memcpy(tree->nodes[i].hash, leaf_hashes[i], SM3_DIGEST_SIZE);
...
// 递归生成审计路径
static void generate_audit_path(uint8_t **hashes, size_t start, size_t end, 
    ...
    if (target_index < start + k) {
        ...
        generate_audit_path(hashes, start, start + k, target_index, proof);
    } else {
        // 目标在右子树
        uint8_t left_hash[SM3_DIGEST_SIZE];
        ...
        proof->path_length++;
        generate_audit_path(hashes, start + k, end, target_index, proof);
    }
}

int merkle_tree_generate_inclusion_proof(merkle_tree_t *tree, size_t leaf_index,
                                        merkle_audit_path_t *proof) {
    if (!tree || !proof || leaf_index >= tree->leaf_count || !tree->nodes) {
        return -1;
    }
    ...
    for (size_t i = 0; i < tree->leaf_count; i++) {
        leaf_hashes[i] = tree->nodes[i].hash;
    }
    
    // 生成审计路径
    generate_audit_path(leaf_hashes, 0, tree->leaf_count, leaf_index, proof);
